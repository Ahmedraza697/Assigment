var questions = [
  ...
  function matchAnswers() {
    let html = `
      <h2 class="match-title">Match Your Answers</h2>
      <div class="match-container">
        <div class="answer-column">
          <h3 style="color:green;">Your Answers</h3>
          <ul>`;
    for (let i = 0; i < selectedAnswers.length; i++) {
      let isCorrect = selectedAnswers[i] === questions[i].correctOption;
      let className = isCorrect ? "correct" : "incorrect";
      html += `<li class="${className}"><b>${i + 1}.</b> ${selectedAnswers[i]}</li>`;
    }
    html += `</ul></div>
        <div class="answer-column">
    ...
    for (let i = 0; i < questions.length; i++) {
      html += `<li><b>${i + 1}.</b> ${questions[i].correctOption}</li>`;
    }
    html += `</ul></div></div>`;
  ...
  document.getElementById("btn").addEventListener("click", function () {
    ...
    for (let i = 0; i < options.length; i++) {
      if (options[i].checked) {
        ...
        selectedLabel = options[i].nextElementSibling;
        break;
      }
    }
